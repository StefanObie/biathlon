    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {heats.map((heat) => (
          <Link
            key={heat}
            href={`/leagues/${leagueIdNum}/reconcile/${heat}`}
            className="flex h-20 flex-col items-center justify-center gap-1 rounded-md border border-input text-2xl font-bold tabular-nums hover:bg-accent"
          >
            {heat}
          </Link>
        ))}
