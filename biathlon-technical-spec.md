...
Stack: TypeScript · Next.js (App Router) on Vercel · Supabase (Postgres/Auth/Realtime/RLS) · Drizzle · Dexie · `@react-pdf/renderer` · shadcn/ui · Husky + Conventional Commits · ESLint/Prettier.
...
**What it is.** TypeScript SQL toolkit, not an ORM. Schema declared in TS; gives typed queries and generated SQL migrations. Stays close to SQL — no hidden join-inventing query layer.
...
export const athlete = pgTable('athlete', {
  ...
  gender:    genderEnum('gender').notNull(),   // 'M' | 'F'
});
 
const rows = await db
  .select({ no: entry.athleteNo, first: athlete.firstName, lane: entry.swimLane })
  .from(entry)
  .innerJoin(athlete, eq(entry.athleteNo, athlete.athleteNo))
...
```
npx drizzle-kit generate    # diffs schema vs last snapshot, writes .sql — no live DB/Docker needed
npx drizzle-kit migrate     # applies pending migrations
```
 
Generated SQL is checked into git, reviewable before running — important because `points_table` is versioned by season; a 2028 update should be a reviewed migration, not a dashboard edit with no record.
 
**Sits alongside Supabase**, not instead of it: Drizzle owns schema/migrations/queries over the standard Postgres connection string; Supabase JS client still handles Auth and Realtime.
 
**Limitations:** RLS policies aren't modelled in Drizzle's schema — write those as raw SQL in a migration file. Smaller ecosystem than Prisma (occasional GitHub-issue archaeology over polished docs).
 
**Practical caution if mixing with Supabase CLI:** Drizzle's `migrate` tracks its own applied-migrations table, separate from `supabase_migrations.schema_migrations`. Pick one tool as the one that actually applies schema migrations (Drizzle, since it's already in the app) and use the Supabase CLI only for RLS policies and local dev stack — otherwise the two migration histories diverge silently.
...
```tsx
const d = QRCode.toString(`SAB-${athleteNo}`, { type: 'svg', errorCorrectionLevel: 'Q' });
...
*Rejected:* `localStorage` (synchronous, blocks main thread, 5MB cap, no querying). Raw IndexedDB (callback-heavy, common source of subtle bugs). Native app (store distribution/per-platform builds for no capability gain — `getUserMedia` already gives full camera access).
...
*Rejected:* server autoincrement (impossible client-side); UUIDv4 (random ordering fragments the index for no benefit here).
...
*Rejected:* `html5-qrcode` — wraps zxing anyway, ships an opinionated UI awkward to strip to a bare continuous-scan view.
...
- **Migration ownership stays with Drizzle** (§5.5) — the Supabase CLI's migration tooling is used only for RLS policy files and local dev stack management, per the caution already noted in §5.5, to avoid two divergent migration histories.
...
1. **XML contract.** XSD or documented format from SA Biathlon? Does `mm:SS.ss` match expectation? Behaviour for an athlete with only one of the two times — empty element, omitted element, or sentinel? *Blocks Phase 2.*
