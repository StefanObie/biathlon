...
**v1.2 · 7 September 2026 · Internal / developer reference**
...
**Finding 7 — scale confirmed against the entry sample.** Max 1,000 athletes/league, one session/league, max 20/heat. The 90-row sample: run heats sized 7–12, swim heats sized 2–6, no duplicate athlete numbers, no (heat, lane) clashes. At this volume Postgres needs no special indexing or partitioning — a plain composite index on `(league_id, run_heat)` and `(league_id, swim_heat, swim_lane)` is sufficient.
...
- **Athlete bibs:** QR payload `BCL-{athleteNo}` (short prefix keeps modules large and scan-fast, and lets the scanner reject foreign QR codes), athlete number in large human-readable digits, name, run heat, swim heat, swim lane. Laid out as a 3-up tag grid (three bibs across an A4 page, sized in mm), sorted by athlete name for easier physical handout. QR ECC level Q, 25mm square, quiet zone maintained.
- **Position cards:** pre-existing, reusable, numbered 1–20 — not generated by this system. **Not scanned** — see §4.3.
...
<results>
  <result>
    ...
    <athleteName>Firstname Lastname</athleteName>
  </result>
</results>
```

**Provisional — blocked on §9 item 1.** `athlete.full_name` is stored as one field (§6.1) because the real entry export has a single "Athlete name" column with no reliable split rule (names like "Van Der Merwe" don't split on a fixed word boundary), and matching for import already keys off athlete number / (swim heat, lane), never name (§4.6). The sample above passes `full_name` through as one `<athleteName>` element rather than inventing a `<athleteName>`/`<athleteSurname>` split. If SA Biathlon's actual contract (§9 item 1) requires two separate name elements, that split has to be sourced from somewhere other than this data — resolve the contract question first rather than guessing at a split now.

**UTF-8 mandatory** — real data contains accented characters such as `é` and `ü`. Use `xmlbuilder2`, never string templates (§5.10).
...
Browser print CSS is unreliable — "shrink to fit" can silently rescale a 25mm QR code by a millimetre or two depending on the volunteer's print dialog, an invisible failure mode. Fixed-dimension PDF prints identically everywhere.
...
Crisp module edges at any print size — soft edges cost decode attempts on every scan of the day. Page dimensions set explicitly in mm; bibs render three across an A4 page. Built-in Helvetica covers Latin-1, so `é`/`ü` render without a custom font. Node runtime only (§5.2).
...
2. Mobile data at outdoor venues over a multi-hour league day isn't perfectly reliable; a dropped packet mid-heat must not lose a capture.
...
- **`@supabase/ssr`**, not the deprecated auth-helpers package, for cookie-based session handling in Server Components/Route Handlers — this is what the `with-supabase` starter (bootstrap line, top of doc) already wires up; don't hand-roll an alternative. Sign-in itself is email + OTP, not password (§5.3) — `@supabase/ssr` still owns the resulting session cookie either way.
...
- **Server Components by default**; `'use client'` only on the two capture screens, the OTP login form, and any component using browser APIs (`getUserMedia`, `navigator.vibrate`, Dexie) — keeps the phone bundle small, per §5.2's existing rationale.
...
**Name storage:** `athlete.full_name` is one column, not `first_name`/`last_name`. The real entry export has a single "Athlete name" column with no reliable split rule (e.g. "Van Der Merwe"), and no import or export path ever keys off name content — resolution is always athlete number or (swim heat, lane) (§4.6), and XML export passes `full_name` through as-is (§4.7). Splitting into two columns would mean inventing a split rule the source data doesn't support.

```sql
create type gender as enum ('M','F');

athlete      (athlete_no pk, full_name, gender)

league       (id pk, name, league_date, season)

entry        (league_id fk, athlete_no fk,
...
Confirmed limits: ≤1,000 athletes/league, one session/league, ≤20/heat (validated against real 90-row sample, §2 Finding 7). Because there's only ever one session per league, `run_heat`/`swim_heat` stay as plain integers on `entry` — no separate `session` or `heat` table. `heat_id` referenced below is the pair `(league_id, run_heat)`.
...
run_result       (league_id, athlete_no, heat_id, run_time, run_time_cs,
                  status, source, overridden_by, override_reason)
swim_result      (league_id, athlete_no, event_no, heat, lane, distance_m,
...
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **0** | Project bootstrap (`create-next-app -e with-supabase`), tooling (shadcn/ui, ESLint/Prettier, Husky + commitlint), entry import, roster, bib PDF generation, OTP operator login | 2–3 wks | Standalone deployable, zero race-day risk. Tooling setup (§5.12–§5.16) front-loaded here so every later phase inherits it. Position cards (§4.1) are pre-existing and out of scope — not generated by this system. |
...
- Power banks for both phones (camera is the dominant battery draw; scanning phone won't survive a full league day unaided).
