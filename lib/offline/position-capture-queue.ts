 ...
 * never-edit-a-capture invariant that applies server-side (§1) applies here
 * too, since these rows are pushed verbatim. */
export interface LocalPositionCapture extends PositionCaptureRow {
  synced: boolean;
}
...
interface StoredCapture extends Omit<PositionCaptureRow, never> {
  synced: 0 | 1;
}

function toStored(row: LocalPositionCapture): StoredCapture {
  return { ...row, synced: row.synced ? 1 : 0 };
}

function fromStored(row: StoredCapture): LocalPositionCapture {
  return { ...row, synced: row.synced === 1 };
}

class PositionCaptureDB extends Dexie {
  position_captures!: EntityTable<StoredCapture, "id">;

  constructor() {
    super("biathlon-position-capture");
    this.version(1).stores({
      // Compound index for the (league_id, run_heat) scoped queries, plus
      // `synced` for the sync sweep's pending-rows lookup.
      position_captures: "id, [league_id+run_heat], synced",
    });
  }
}

const db = new PositionCaptureDB();

export async function putCapture(row: LocalPositionCapture): Promise<void> {
  await db.position_captures.put(toStored(row));
}
...
export async function voidCapture(
  ...
  await db.position_captures.update(id, {
    voided: true,
...
export async function getCapturesForHeat(
  leagueId: number,
  runHeat: number,
): Promise<LocalPositionCapture[]> {
  const rows = await db.position_captures
...
async function getUnsyncedCaptures(): Promise<LocalPositionCapture[]> {
  const rows = await db.position_captures.where("synced").equals(0).toArray();
  return rows.map(fromStored);
}
...
export async function syncPendingCaptures(): Promise<void> {
  ...
  try {
    ...
    const rows: PositionCaptureRow[] = pending.map((row) => ({
      id: row.id,
      league_id: row.league_id,
      run_heat: row.run_heat,
      position: row.position,
      athlete_no: row.athlete_no,
